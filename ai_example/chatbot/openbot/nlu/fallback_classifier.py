...
RESPONSE_SELECTOR_THRESHOLD_KEY = "response_selector_threshold"
DEFAULT_RESPONSE_SELECTOR_THRESHOLD = 0.3
...
class FallbackClassifier(GraphComponent, IntentClassifier):
    ...
    def get_default_config() -> Dict[Text, Any]:
        ...
        return {
            ...
            RESPONSE_SELECTOR_THRESHOLD_KEY: DEFAULT_RESPONSE_SELECTOR_THRESHOLD
        }
    ...
    def _should_fallback(self, message: Message) -> bool:
        ...
        response_selector_below_threshold, rs_confidence = self._response_selector_confidence_below_threshold(
            message)

        if response_selector_below_threshold:
            logger.debug(
                f"Response Selector confidence {rs_confidence} for intent '{intent_name}' is lower "
                f"than Response Selector threshold {self.component_config[RESPONSE_SELECTOR_THRESHOLD_KEY]:.2f}."
            )
            return True
    ...
    def _response_selector_confidence_below_threshold(self, message: Message) -> Tuple[bool, float]:
        print (message.data)
        intent = message.data['intent']['name']
        if intent in message.data['response_selector']['all_retrieval_intents']:
            response_selector_confidence = message.data['response_selector']['default']['response']['confidence']
            return response_selector_confidence < self.component_config[
                RESPONSE_SELECTOR_THRESHOLD_KEY], response_selector_confidence
